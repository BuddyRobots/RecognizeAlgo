...
namespace CardDetection
{
    public class FeatureTracker
    {
        ...
        public Mat process(Mat frame)
        {
            ...
            if (frame_desc.rows() < 2 || frame_desc.cols() < 2) return resultImg;
            ...
            {
                if (matches[i].toList()[0].distance < nn_match_ratio * matches[i].toList()[1].distance && matches[i].toList()[0].distance < 50)
                {
            ...
            if (matched_tem.Count < 4) return resultImg;
            ...
            if (inliers_fra.rows() == 0) return resultImg;
            Mat new_homography = Calib3d.findHomography(inliers_tem, inliers_fra);

            // Perspective transform to get frame bounding box
            List<Point> l_frame_bb = utils.perTrans(templ_bb, new_homography);
